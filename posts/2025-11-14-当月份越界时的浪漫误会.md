...
非严格模式下，它对 **日** 特别宽容：
 1 到 31，无论月份是否真的有这么多天，它都轻轻放过。
 但对于 **月、年、时分秒**，它依旧保持警觉，任何超出边界都会被拒绝。

这就像一位老师：

> “孩子，你的日子我不苛求，哪怕多一天或少一天，我都微笑着让你过去；
>  但月与年啊，那是规矩，我不会让你随便编造。”

于是 `"2023-11-31"` 能被解析，而 `"2023-13-01"` 仍被挡在门外。
 温柔中带着锋芒，像一首轻吟的小诗，提醒我们：**偶尔的自由，需要在规则中被理解。**
...
表面上它和 `uuuu` 一样，都是四位年份。
 但是，`yyyy` 表示的是 **year-of-era**，它和纪元（Era）绑定，偶尔会在解析 `LocalDateTime` 时产生奇怪报错。
...
- 严格模式下，它像守护者，提醒我们真实的存在；
- 非严格模式下，它又像温柔的老师，对日子偶尔宽容；
- 微小的符号差异，也能影响解析的成败。
